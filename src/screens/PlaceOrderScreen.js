...
const PlaceOrderScreen = () => {
    ...
    return (
        <main className=" h-screen banner my-12 mb-40">
            <div className="max-w-screen-xl py-12 mx-auto px-4">
                <Back />
                {order.length > 0 ? (
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                            ...
                            {/* right side  */}
                            <div className="col-span-1">
                                <div className="glass p-4 box-border rounded-lg">
                                    {/* order details  */}
                                    <div className="flex flex-col space-y-4 mb-3 ">
                                        <p className="poppins text-gray-700 flex justify-between">Deliver Place :  <span className="font-semibold text-black">{input.country ? `${input.country}` : '-----'}</span></p>
                                        <p className="poppins text-gsray-700 flex justify-between">Ariving in 20-30 min</p>
                                        <p className="poppins text-gray-700 flex justify-between">Road <span className="font-semibold text-black">{input.roadNo ? `${input.roadNo}` : '-----'}</span> </p>
                                        <p className="poppins text-gray-700 flex justify-between">Floor :  <span className="font-semibold text-black">{input.flatno ? `${input.flatno}` : '-----'}</span> </p>
                                        <p className="poppins text-gray-700 flex justify-between">Deliver to :  <span className="font-semibold text-black">{input.name ? `${input.name}` : '-----'}</span> </p>
                                    </div>
                                    {/* orders  */}
